#!/usr/bin/env -S deno run --ext=ts --allow-read --allow-write --allow-env --allow-net --allow-run --unstable
...
import { Manifest } from "https://esm.sh/webextension-polyfill-ts@0.26.0";
import { assert } from "https://deno.land/std@0.213.0/assert/mod.ts";

const projectPath = new URL(".", import.meta.url).pathname;

async function shell(procName: string, argsArray: string[] = []) {
...
function createFirefoxManifest(manifest: Manifest.WebExtensionManifest) {
  ...
  delete (manifest.background as {service_worker?: any})!["service_worker"];
...
async function buildStorePackage() {
  ...
  const chromeManifest: Manifest.WebExtensionManifest = await parseManifestFile();
  const rsyncOptions = ["-r", ".", "dist/vimium"].concat(
    ...excludeList.map((item) => ["--exclude", item]),
  );
  const vimiumVersion = chromeManifest["version"];
  const writeDistManifest = async (manifest: Manifest.WebExtensionManifest) => {
...
const runDomTests = async (port: number) => {
...
task("fetch-tlds", [], async () => {
  ...
  const domains = Array.from(els).map((el) => (el as Element).innerText.slice(1));
